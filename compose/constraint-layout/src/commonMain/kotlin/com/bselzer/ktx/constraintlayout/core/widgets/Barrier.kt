    ...
    fun allSolved(): Boolean {
        ...
        if (hasAllWidgetsResolved && mWidgetsCount > 0) {
            ...
            for (i in 0 until mWidgetsCount) {
                ...
                if (barrierType == LEFT) {
                    barrierPosition = min(barrierPosition, widget.getAnchor(ConstraintAnchor.Type.LEFT)!!.finalValue)
                } else if (barrierType == RIGHT) {
                    barrierPosition = max(barrierPosition, widget.getAnchor(ConstraintAnchor.Type.RIGHT)!!.finalValue)
                } else if (barrierType == TOP) {
                    barrierPosition = min(barrierPosition, widget.getAnchor(ConstraintAnchor.Type.TOP)!!.finalValue)
                } else if (barrierType == BOTTOM) {
                    barrierPosition = max(barrierPosition, widget.getAnchor(ConstraintAnchor.Type.BOTTOM)!!.finalValue)
                }
            }
