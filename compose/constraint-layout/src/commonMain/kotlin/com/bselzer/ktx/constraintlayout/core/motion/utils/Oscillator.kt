...
class Oscillator constructor() {
    ...
    fun addPoint(position: Double, period: Float) {
        ...
        var j: Int = mPosition.toList().binarySearch(position)
        if (j < 0) {
            j = -j - 1
        }
    ...
    fun getP(time: Double): Double {
        ...
        var index: Int = mPosition.toList().binarySearch(time)
        var p: Double = 0.0
        if (index > 0) {
            p = 1.0
        } else if (index != 0) {
    ...
    fun getValue(time: Double, phase: Double): Double {
        ...
        when (mType) {
            SIN_WAVE -> return sin(PI2 * (angle))
            SQUARE_WAVE -> return signum(0.5 - angle % 1)
            TRIANGLE_WAVE -> return 1 - abs(((angle) * 4 + 1) % 4 - 2)
            SAW_WAVE -> return ((angle * 2 + 1) % 2) - 1
            REVERSE_SAW_WAVE -> return (1 - ((angle * 2 + 1) % 2))
            COS_WAVE -> return cos(PI2 * (phase + angle))
            BOUNCE -> {
                val x: Double = 1 - abs(((angle) * 4) % 4 - 2)
                return 1 - x * x
            }
            CUSTOM -> return mCustomCurve!!.getPos(angle % 1, 0)
            else -> return sin(PI2 * (angle))
        }
    }
    ...
    fun getSlope(time: Double, phase: Double, dphase: Double): Double {
        ...
        when (mType) {
            SIN_WAVE -> return PI2 * dangle_dtime * cos(PI2 * angle)
            SQUARE_WAVE -> return 0.0
            TRIANGLE_WAVE -> return 4 * dangle_dtime * signum(((angle) * 4 + 3) % 4 - 2)
            SAW_WAVE -> return dangle_dtime * 2
            REVERSE_SAW_WAVE -> return -dangle_dtime * 2
            COS_WAVE -> return -PI2 * dangle_dtime * sin(PI2 * angle)
            BOUNCE -> return 4 * dangle_dtime * (((angle) * 4 + 2) % 4 - 2)
            CUSTOM -> return mCustomCurve!!.getSlope(angle % 1, 0)
            else -> return PI2 * dangle_dtime * cos(PI2 * angle)
        }
    }
