...
import com.bselzer.ktx.constraintlayout.utility.Utils.pow

class Utils constructor() {
    fun getInterpolatedColor(value: FloatArray): Int {
        val r: Int = clamp(
            (pow(value.get(0).toDouble(), 1.0 / 2.2).toFloat() * 255.0f).toInt()
        )
        val g: Int = clamp(
            (pow(value.get(1).toDouble(), 1.0 / 2.2).toFloat() * 255.0f).toInt()
        )
        val b: Int = clamp(
            (pow(value.get(2).toDouble(), 1.0 / 2.2).toFloat() * 255.0f).toInt()
        )
