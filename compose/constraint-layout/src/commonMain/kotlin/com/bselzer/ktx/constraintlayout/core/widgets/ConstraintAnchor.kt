...
import com.bselzer.ktx.constraintlayout.core.widgets.analyzer.Grouping.findDependents
import com.bselzer.ktx.constraintlayout.core.widgets.analyzer.WidgetGroup
 ...
 */(
    ...
    var dependents: HashSet<ConstraintAnchor>? = null
        private set
    ...
    fun findDependents(orientation: Int, list: ArrayList<WidgetGroup>, group: WidgetGroup?) {
        if (dependents != null) {
            for (anchor in dependents!!) {
                findDependents(anchor.owner, orientation, list, group)
            }
        }
    }
    ...
    fun copyFrom(source: ConstraintAnchor, map: HashMap<ConstraintWidget?, ConstraintWidget?>) {
        ...
        if (source.target != null) {
            ...
            val owner = map[source.target!!.owner]
