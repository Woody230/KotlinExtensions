 ...
 */
package com.bselzer.ktx.constraintlayout.core.motion.utils

import com.bselzer.ktx.constraintlayout.core.motion.CustomAttribute
import com.bselzer.ktx.constraintlayout.core.motion.CustomVariable
...
import com.bselzer.ktx.constraintlayout.utility.Utils.signum
import kotlin.math.*
...
abstract class TimeCycleSplineSet constructor() {
    ...
    protected fun calcWave(period: Float): Float {
        val p: Float = period
        when (mWaveShape) {
            Oscillator.Companion.SIN_WAVE -> return sin((p * VAL_2PI).toDouble()).toFloat()
            Oscillator.Companion.SQUARE_WAVE -> return signum(p * VAL_2PI)
            Oscillator.Companion.TRIANGLE_WAVE -> return 1 - abs(p)
            Oscillator.Companion.SAW_WAVE -> return ((p * 2 + 1) % 2) - 1
            Oscillator.Companion.REVERSE_SAW_WAVE -> return (1 - ((p * 2 + 1) % 2))
            Oscillator.Companion.COS_WAVE -> return cos((p * VAL_2PI).toDouble()).toFloat()
            Oscillator.Companion.BOUNCE -> {
                val x: Float = 1 - abs((p * 4) % 4 - 2)
                return 1 - x * x
            }
            else -> return sin((p * VAL_2PI).toDouble()).toFloat()
        }
    }
    ...
    companion object {
        ...
        protected var VAL_2PI: Float = (2 * PI).toFloat()
    }
}