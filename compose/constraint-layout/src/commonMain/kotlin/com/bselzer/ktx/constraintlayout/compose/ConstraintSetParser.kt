...
import com.bselzer.ktx.constraintlayout.core.motion.utils.TypedBundle
import com.bselzer.ktx.constraintlayout.core.motion.utils.TypedValues
import com.bselzer.ktx.constraintlayout.core.parser.*
import com.bselzer.ktx.constraintlayout.core.state.ConstraintReference
import com.bselzer.ktx.constraintlayout.core.state.Dimension
import com.bselzer.ktx.constraintlayout.core.state.Dimension.Companion.Fixed
...
import com.bselzer.ktx.constraintlayout.core.state.Dimension.Companion.Wrap
import com.bselzer.ktx.constraintlayout.core.state.HelperReference
import com.bselzer.ktx.constraintlayout.core.state.State.Chain.*
import com.bselzer.ktx.constraintlayout.core.state.Transition
import com.bselzer.ktx.constraintlayout.core.state.helpers.GuidelineReference
...
internal fun parseJSON(content: String, state: State, layoutVariables: LayoutVariables) {
    try {
        ...
        (0 until elements.size).forEach { i ->
            ...
            val element = json[elementName]
            if (PARSER_DEBUG) {
                println("element <$elementName = $element> " + element::class)
            }
            when (elementName) {
        ...
        when (elementName) {
            "Design" -> {
                ...
                (0 until elements.size).forEach { i ->
                    ...
                    println("element found <$elementName>")
...
internal fun parseBarrier(
    ...
    val reference =
        state.barrier(elementName, com.bselzer.ktx.constraintlayout.core.state.State.Direction.END)
    val constraints = element.names() ?: return
    (0 until constraints.size).forEach { i ->
        when (val constraintName = constraints[i]) {
            "direction" -> {
                when (element.getString(constraintName)) {
                    "start" -> reference.setBarrierDirection(com.bselzer.ktx.constraintlayout.core.state.State.Direction.START)
                    "end" -> reference.setBarrierDirection(com.bselzer.ktx.constraintlayout.core.state.State.Direction.END)
                    "left" -> reference.setBarrierDirection(com.bselzer.ktx.constraintlayout.core.state.State.Direction.LEFT)
                    "right" -> reference.setBarrierDirection(com.bselzer.ktx.constraintlayout.core.state.State.Direction.RIGHT)
                    "top" -> reference.setBarrierDirection(com.bselzer.ktx.constraintlayout.core.state.State.Direction.TOP)
                    "bottom" -> reference.setBarrierDirection(com.bselzer.ktx.constraintlayout.core.state.State.Direction.BOTTOM)
                }
            }
            "margin" -> {
