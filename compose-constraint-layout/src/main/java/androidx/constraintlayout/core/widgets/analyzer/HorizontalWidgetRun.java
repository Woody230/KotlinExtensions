...
public class HorizontalWidgetRun extends WidgetRun {
            ...
            if (super.dimensionBehavior != ConstraintWidget.DimensionBehaviour.MATCH_CONSTRAINT) {
                if (dimensionBehavior == ConstraintWidget.DimensionBehaviour.MATCH_PARENT) {
                    ...
                    if (parent != null
                            ...
                            || parent.getHorizontalDimensionBehaviour() == ConstraintWidget.DimensionBehaviour.MATCH_PARENT)) {
    ...
    public void update(Dependency dependency) {
        switch (mRunType) {
            ...
            break;
            case CENTER: {
                updateRunCenter(dependency, widget.mLeft, widget.mRight, ConstraintWidget.HORIZONTAL);
                return;
            }
            default: break;
        }
                        ...
                        if (widget.mMatchConstraintDefaultHeight == ConstraintWidget.MATCH_CONSTRAINT_SPREAD
                            ...
                            if (s1 && s2 && e1 && e2) {
                                ...
                                switch (definedSide) {
                                    ...
                                    case ConstraintWidget.HORIZONTAL: {
                                        int dx = x2 - x1;
                                        int ldx = getLimitedDimension(dx, ConstraintWidget.HORIZONTAL);
                                        int dy = (int) (0.5f + ldx * ratio);
                                        int ldy = getLimitedDimension(dy, ConstraintWidget.VERTICAL);
                                        if (dy != ldy) {
