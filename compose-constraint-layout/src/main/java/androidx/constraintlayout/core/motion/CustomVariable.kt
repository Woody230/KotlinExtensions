...
class CustomVariable {
    ...
    fun setValue(value: FloatArray) {
        when (type) {
            ...
            TypedValues.Custom.TYPE_COLOR -> {
                ...
                val f_g = value[1]
                val f_b = value[2]
                val r = 0xFF and Math.round(Math.pow(f_r.toDouble(), 1.0 / 2.0).toFloat() * 255.0f)
                val g = 0xFF and Math.round(Math.pow(f_g.toDouble(), 1.0 / 2.0).toFloat() * 255.0f)
                val b = 0xFF and Math.round(Math.pow(f_b.toDouble(), 1.0 / 2.0).toFloat() * 255.0f)
                val a = 0xFF and Math.round(value[3] * 255.0f)
    ...
    fun getInterpolatedColor(value: FloatArray): Int {
        val r = clamp((Math.pow(value[0].toDouble(), 1.0 / 2.2).toFloat() * 255.0f).toInt())
        val g = clamp((Math.pow(value[1].toDouble(), 1.0 / 2.2).toFloat() * 255.0f).toInt())
        val b = clamp((Math.pow(value[2].toDouble(), 1.0 / 2.2).toFloat() * 255.0f).toInt())
    ...
    companion object {
        ...
        fun colorString(v: Int): String {
            val str = "00000000" + Integer.toHexString(v)
