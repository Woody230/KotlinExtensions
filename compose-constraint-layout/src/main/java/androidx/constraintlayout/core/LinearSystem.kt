...
class LinearSystem {
    ...
    private fun increaseTableSize() {
        ...
        if (metrics != null) {
            ...
            metrics!!.maxTableSize = Math.max(metrics!!.maxTableSize, TABLE_SIZE.toLong())
    ...
    fun minimizeGoal(goal: Row?) {
        if (metrics != null) {
            ...
            metrics!!.maxVariables = Math.max(metrics!!.maxVariables, mNumColumns.toLong())
            metrics!!.maxRows = Math.max(metrics!!.maxRows, numEquations.toLong())
        }
...
"""
    ...
    fun addCenterPoint(widget: ConstraintWidget, target: ConstraintWidget, angle: Float, radius: Int) {
        ...
        var row = createRow()
        var angleComponent = (Math.sin(angle.toDouble()) * radius).toFloat()
        row.createRowWithAngle(At, Ab, Bt, Bb, angleComponent)
        addConstraint(row)
        row = createRow()
        angleComponent = (Math.cos(angle.toDouble()) * radius).toFloat()
